...
exports.activate = exports.registerHeroTreeView = void 0;
...
class HeroProvider {
    ...
    async loadHeroes() {
        try {
            const response = await axios_1.default.get('https://game.gtimg.cn/images/lol/act/img/js/heroList/hero_list.js?ts=2794914');
            const data = response.data;
            if (data && data.hero) {
                this.heroes = data.hero.map((hero) => ({
                    ...hero,
...
function registerHeroTreeView(context) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('showHeroDetails', async (hero) => {
        try {
            ...
            const panel = vscode.window.createWebviewPanel('heroDetails', `英雄 - ${hero.name}`, vscode.ViewColumn.One, {});
            // 映射表将英文角色属性映射到对应的中文值
            ...
            const roles = heroDetails.roles.map((role) => roleMapping.get(role) || role).join('、');
            const htmlContent = `
                <!DOCTYPE html>
                <html>
...
class ItemProvider {
    ...
    async loadItems() {
        try {
            const response = await axios_1.default.get('https://game.gtimg.cn/images/lol/act/img/js/items/items.js?ts=2794915');
            const data = response.data;
            if (data && data.items) {
                // 过滤掉 price 为 0 的数据
                this.items = data.items.filter((item) => item.price !== "0");
                this._onDidChangeTreeData.fire(undefined);
            }
        }
        catch (error) {
...
class RuneProvider {
    ...
    async loadRunes() {
        try {
            const response = await axios_1.default.get('https://game.gtimg.cn/images/lol/act/img/js/runeList/rune_list.js');
            const data = response.data;
            if (data && data.rune) {
                this.runes = Object.values(data.rune).map((rune) => rune);
                this._onDidChangeTreeData.fire(undefined);
            }
        }
        catch (error) {
...
const itemTreeView_1 = __webpack_require__(43);
const runeTreeView_1 = __webpack_require__(44);
function activate(context) {
